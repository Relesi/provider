...
public class OrderDemandService {
	...
	private List<OrderItem> toorderItems(List<OrderItemDTO> items) {
		...
		List<Long> idsProducts = items.stream().map(item -> item.getId()).collect(Collectors.toList());

		List<Product> productsOrder = productRepository.findByIdIn(idsProducts);

		List<OrderItem> orderItems = items.stream().map(item -> {
			Product product = productsOrder.stream().filter(p -> p.getId() == item.getId()).findFirst().get();
